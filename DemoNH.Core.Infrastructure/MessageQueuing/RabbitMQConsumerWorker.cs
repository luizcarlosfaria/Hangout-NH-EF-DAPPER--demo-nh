...
namespace DemoNH.Core.Infrastructure.MessageQueuing
{
	public class RabbitMQConsumerWorker : IQueueConsumerWorker
	{
		...
		public RabbitMQConsumerWorker(IConnection connection, string queueName, IMessageProcessingWorker messageProcessingWorker, IMessageRejectionHandler messageRejectionHandler, ISerializer serializer, Func<int> scaleCallbackFunc, Type expectedType, CancellationToken parentToken)
		{
			...
			Contract.Requires(serializer != null, "serializer is required");
			...
			while (!this._cancellationTokenSource.Token.IsCancellationRequested)
			{
				...
				if (lastResult != null)
				{
					...
					try
					{
						...
						messageObject = this._serializer.Deserialize(messageBody, this._expectedType);
					}
